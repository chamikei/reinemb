...
class MiniImageNet(Dataset):
    def __init__(self, setname, args, augment: bool = False, data_path: str = 'data/miniimagenet'):
        im_size = args.orig_imsize
        self.data_path = data_path
        csv_path = osp.join(self.data_path, 'split', setname + '.csv')
        self.use_im_cache = (im_size != -1)
        if self.use_im_cache:
            cache_path = osp.join(self.data_path, 'cache_{}_{}_{}.pt'.format('MiniImageNet', setname, im_size))
            if not osp.exists(cache_path):
                print('Cache miss ... Preprocessing {} {} ...'.format('miniimagenet', setname))
    ...
    def parse_csv(self, csv_path):
        lines = [x.strip() for x in open(csv_path, 'rt').readlines()][1:]
        data, label, wnids = [], [], []
        lb = -1
        for l in lines:
            name, wnid = l.split(',')
