...
class DND:
    ...
    def save_memory(self, memory_key, memory_val):
        ...
        if self.encoding_off:
            return
        self.keys.append(torch.squeeze(memory_key.detach()))
        self.vals.append(torch.squeeze(memory_val.detach()))
...
def compute_similarities(query_key, key_list, metric):
    ...
    """
    q = query_key.detach().view(1, -1)
    M = torch.stack(key_list)
    if metric == 'cosine':
        similarities = F.cosine_similarity(q, M)
    elif metric == 'l1':
        similarities = -1. * F.pairwise_distance(q, M, p=1)
    elif metric == 'l2':
        similarities = -1. * F.pairwise_distance(q, M, p=2)
    else:
...
def _empty_memory(memory_dim):
    ...
    """
    return torch.zeros((1, memory_dim), device=torch.device('cuda'))
